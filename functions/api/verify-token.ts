...
export interface Env {
  SUPABASE_URL: string;
  SUPABASE_ANON_KEY: string; // Required for user-scoped Supabase client
  SUPABASE_SERVICE_ROLE_KEY?: string; // Optional, only for admin operations
  SUPABASE_JWT_SECRET?: string; // Optional fallback for HS256 tokens (legacy)
  SUPABASE_JWKS_URL?: string; // Optional, defaults to {SUPABASE_URL}/auth/v1/.well-known/jwks.json
}
...
/**
 * Verify JWT token signature using JWKS (production-ready)
 * Falls back to basic validation if JWKS fails (for Cloudflare Workers compatibility)
 */
async function verifyTokenSignature(token: string, env: Env): Promise<VerifyResult> {
  try {
    ...
    try {
      ...
      // JWKS failed (e.g., network issue in Cloudflare Workers), fallback to basic validation
      console.warn('[verify-token] JWKS verification failed, using fallback:', jwksError.message);
 ...
 * Attempts signature verification via JWKS, falls back to basic validation if JWKS fails
 */
export async function verifySupabaseToken(token: string, env: Env): Promise<VerifyResult> {
  return verifyTokenSignature(token, env);
}
